...
** *Tools* -> *Options* -> General -> *EDA Tool Options* -> EDA Tool: Questa Intel FPGA : carpeta donde se localiza la aplicación Questa
...
* Editar el fichero 'light_bit.vhd'
+
...
* Descripción del circuito *z = x~0~ XOR x~1~* 
+
...
----
* Se ha utilizado para los buses de señales el tipo bit_vector.
...
* Cómo se ejecuta la sentencia asignación concurrente '<='
** A las señales de la derecha del símbolo '<=' se les denomina *señales sensibles*
** Cuando una de la señales sensibles cambia (*evento*) en ese mismo instante se ejecuta la sentencia asignación.
...
Ejemplo: Unidad de diseño "matricula.vhd"
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
begin
    ...
    b 		<= SW(1) and SW(2);		<2>
    c 		<= SW(3);			<2>
    LEDR(0) 	<= a or b or c;			<2>
end puertas;
----

* Verificar el buen funcionamiento del circuito con su simulación.


Significado
~~~~~~~~~~~

<1>  la declaración *signal* se utiliza para declarar señales internas a la arquitectura, a diferencia de los puertos de la entidad que son señales externas al circuito. Las señales 'a', 'b' y 'c' son internas.
<2>  Las sentencias *<=* se ejecutan en el instante en que alguna de las señales a la derecha de *<=* cambia su valor. Por lo tanto las 4 sentencias no se ejecutan secuencialmente, sino CONCURRENTEMENTE. Por ejemplo, si el valor de la señal externa 'SW(1)' cambia en un momento dado de "0" a "1" , entonces, las asignaciones <3> y <4> se ejecutan en paralelo, concurrentemente, al mismo tiempo, por lo que el valor de las señales 'a' y 'b' también cambia y por lo tanto 'LEDR(0)' también cambiará; en resumen, el cambio en 'SW(1)' puede provocar un cambio simultáneo en 'a','b','LEDR(0)'...lo mismo que ocurriría en un circuito digital electrónico de puertas lógicas donde todas las puertas lógicas cambian sus salidas en el instante en el que lo hacen sus entradas.
...
** ..vhdl_src/std/standard.vhdl

* La librería std y sus paquetes están disponibles en la carpeta donde se ha instalado el simulador Modelsim-Altera
** ../modelsim_ase/vhdl_src/std/standard.vhdl
...
** el nivel desconocido se divide en forzoso, débil tirando a 1 y débil tirando a 0.
...
* La librería std y sus paquetes están disponibles en la carpeta donde se ha instalado el simulador Questa Intel
** ..vhdl_src/std/textio.vhdl

* La librería std y sus paquetes están disponibles en la carpeta donde se ha instalado el simulador Modelsim-Altera
** ../modelsim_ase/vhdl_src/std/textio.vhdl
...
Tutorial quartus -> light.vhd (IV) : Simulación Questa Intel
------------------------------------------------------------
...
*** Quartus -> Tools -> options -> general -> eda tool options -> Questa Intel : seleccionar la carpeta donde esté instalado, por ejemplo -> C:/intel_quartus/21.1/questa_fse/bin
** generar fuentes del simulador
*** Processing -> Start Compilation
** Abrir el simulador Questa Intel
*** Tools -> Run Simulator Tool -> RTL simulation -> Abre el simulador Questa Intel
...
Edición VHDL mediante Quartus utilizando plantillas de ayuda
------------------------------------------------------------
...
* Arquitectura mediante la sentencia concurrente *process*
** En el cuerpo del proceso se utilizarán la sentencia de asignación secuencial "<=".
* Ejercicio 1.5 del libro de problemas resueltos de circuitos electrónicos
** Unidad 'matricula.vhdl'
+
...
----
** ¿Qué ocurriría si el cuerpo del process tuviese otro orden como el siguiente?


Ejemplo_2: matricula.vhd
~~~~~~~~~~~~~~~~~~~~~~~~
...
Tutorial quartus -> light_if.vhd (VI)
-------------------------------------
...
Tutorial quartus -> light_with.vhd (VII)
----------------------------------------
...
Tutorial quartus -> light_case.vhd (VIII)
-----------------------------------------
...
Decoder Hex/Display_7_segmentos
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
* El paquete std_logic_1164 no define operaciones aritméticas para operar con señales std_logic.
...
image::web_interface/sakai.png[Miaulario]
...
.An example table
[width="100%",cols="2<m,3<m,3m",frame="all",options="header"]
|==========================
