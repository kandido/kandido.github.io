...
----
1. Arquitectura de Von Neumann: 1.1,1.2,1.3,1.4,1.6,2.1,2.2,2.3 (NO programar en el lenguaje IASSim) 
...
6. Programación en el lenguaje Ensamblador AT&T: 1.1,2.1,2.2
----
...
----
1. Arquitectura de Von Neumann: 2.5,2.6
...
b. Do it the “hard way” without using the equation from part (a): latexmath:[$\sum_{i=1}^{5}i$]
** Desarrollo:
+
...
|08A		|  010FA110FB	| AC<-M[0FA]  |M[0FB]<-AC 		|LOAD  M[0FA]  |STORE M[0FB]
|08B		|  010FA0F08D	| AC<-M[0FA]  |AC>0:PC <- 0x08D(0:19)	|LOAD  M[0FA]  |JMP   +M[08D(0:19)]
|08C		|  020FA110FB	| AC<--M[0FA] |M[0FB]<-AC		|LOAD -M[0FA]  |STORE M[0FB]
|08D            .5+| 		 						
|============================================================================
*** Contents: instrucción izda 010FA (Cod Op 01 Address 0FA) e instrucción dcha 110FB -
...
. Escribir el carácter *a* con acento *á* en el código universal UTF-8 (hexadecimal), en el código Unicode(U+XXXX), en ascii extendido e iso-8859-1 consultando el enlace link:https://charset.org/utf-8[UTF-8], los manuales: man ascii, man iso-8859-1,  y el comando +showkey -a+ -> teclear *á*
...
----
** En notación científica: 1.0011010010100100010110000101*2^+10
...
****
Palabra de 32 bits -> Registros de propósito general de 32 bits y Registro de Instrucción IR de 32 bits
...
. Una computadora con palabras de 24 bits posee 16 instrucciones diferentes de un operando, 8 registros  de propósito general, y 3 modos de direccionamiento (directo a registro, indirecto con registro e indirecto con desplazamiento a registro-base)
...
. Una computadora tiene un formato de instrucción de 11 bits donde el campo de operando es de 4 bits. ¿Es posible codificar en este formato 5 instrucciones de dos operandos, 45 de un operando y 48 sin operando?. Justificar la respuesta.
** SOLUCION
** 3 tipos de formatos
...
. Una computadora de 16 bits de ancho de palabra (instrucciones, palabra de memoria, registros) y 8 registros, tiene el siguiente repertorio de instrucciones:
...
. Una computadora basada en el  procesador de Motorola M68000 presenta los siguientes contenidos en registro y memoria:
+
...
. Una computadora presenta los siguientes contenidos de los registros y la memoria: 
+
...
* Si el contenido del desplazamiento de la instrucción en ejecución es 96 ¿Cuál seria el valor del operando con los siguientes direccionamientos?.
	...
	a)Directo de memoria (dir = desp).
	b)Indirecto de memoria (dir memoria = desp).
	c)Directo de registro con R1.
	d)Indirecto de registro con R1.
	e)Indirecto con desplazamiento con registro base R2

. Se tiene una  computadora con un ancho de palabra de 32 bits y con un banco de registros de 32 registros de 32 bits. El computador tiene 64 instrucciones diferentes y los siguientes modos de direccionamiento: Un formato de instrucción con direccionamiento directo de memoria e indirecto de memoria y el otro formato con direccionamiento indirecto con desplazamiento a registro-base.
...
**** Total IR =32 bits -> Memoria=32-(6+1+5)=32-12=20 bits
...
.. Si cada dirección de memoria hace referencia a un byte ¿qué zona de memoria se puede acceder con cada uno de los modos de direccionamiento?
*** SOLUCION
...
. Considerando que en un procesador cada  búsqueda de instrucción y cada acceso a un  operando consumen un ciclo y teniendo en  cuenta para 3 programas los siguientes datos en millones de  referencias. 
+
...
*** CF es el flag de overflow en este caso.
*** Sin signo -> para el caso Destino > Fuente, la resta Destino - Fuente da lugar a   CF=0 y OF según la resta con signo. El flag de signo no vaŕía SF=X.
*** Sin signo -> para el caso Destino < Fuente, la resta Destino - Fuente da lugar a   CF=1 y OF según la resta con signo. El flag de signo no varía SF=X.
...
|===
|Caso|CF|PF|AF|ZF|SF|OF
|Dest>Source|x|x|x|0|0/1|0/1
|Dest=Source|0|x|x|1|0|0
|Dest<Source|1|x|x|0|0/1|1/0
|===
...
. Interpretación de los flags CF y OF con la instrucción suma  aritmética ADD. Al igual que con SUB el flag CF responde al overflow de la suma aritmética SIN signo y el flag OF responde al overflow de la suma aritmética con signo. Comprobarlo con el programa:
+
...
.. declaración de la variable s1:  tipo estructura
+
...
.. declaración de la variable s2:  tipo estructura
+
...
* Explicación breve de las operaciones de los  <<mnemonicos_basicos_explicados, códigos mnemónicos más básicos>>. Para información más detallada ir al <<repertorio_instruc_oper,Manual del Repertorio de Instrucciones de INTEL>>
...
* teniendo en cuenta que la sección de datos  presenta el código siguiente:
+
...
. Analizar la subrutina sumMtoN.s
+
...
bucle:
	...
	.end
----
. Programar una rutina main con llamada a la subrutina sumMtoN. Salir de main con la llamada a sistema exit. Ejecutar el programa con el depurador GDB y comprobar que el comando "next" *entra* en la subrutina sumMtoN. Estando dentro de la subrutina interpretar el comando "backtrace" *bt* que informa sobre el número y estado de los frames del stack.
. Programar una rutina main con llamada a la subrutina sumMtoN. Nada más entrar en main editar el prólogo de main (creación de un nuevo frame) y terminar main con el epílogo (fin del frame) y con la instrucción "ret".  Estando dentro de la subrutina interpretar el comando "backtrace" *bt* que informa sobre el número y estado de los frames del stack.
+
...
. Libro William Stalling Ejemplo 4.2a Pg125. Calcular para una controladora de Caché con Asociación de Mapeo Directo, qué línea de la memoria caché (tag/línea/word) está asociada a la dirección 0x02ABCD de la memoria principal.
**  Formato de la dirección de memoria principal (Bloque MPrincipal/Word)
+
...
<--------s:22bits-----><-w:2bits->
-------------------------------------------------
** Formato de la dirección de memoria principal (Tag/Línea Caché/Word)
+
...
|última    |          |        |       |        |        |
+----------+----------+--------+-------+--------+--------+
           <--8bits-><-1byte-><-1byte-><-1byte-><-1byte->
-------------------------------------------------


** 0x02ABCD = 0000-0010-1010-1011-1100-1101
** Block/Word ->  0000001010101011110011/01 -> Block=0000001010101011110011 Word=01 -> block=0x0AAF3 word=01
** Tag/Line/Word -> 00000010/10101011110011/01 -> Tag=00000010 Line=10101011110011 Word=01 -> tag=02 line=0x2AF3 word=01
*** Tag 0x02 Línea 0x2AF3 Word 1
*** Cuando la CPU lance la dirección 0x02ABCD, la controladora de la Caché irá a la línea 0x2AF3 y si el Tag de esa línea es 0x02 quiere decir que el contenido de datos (4 bytes)  de esa línea es una copia del contenido de datos (4 bytes) del bloque 0x0AAF3 y por lo tanto la palabra 01 de ese bloque de la memoria principal está en esa línea de la caché -> hit. En caso de que en esa línea el tag no sea 02, los datos de esa línea no son una copia del bloque y la controladora de la caché tendrá que pedir a la controladora de la memoria principal una copia del bloque para la caché y otra copia para la CPU.

. Repetir el ejercicio anterior mediante multiplicaciones y divisiones binarias respondiendo a los siguientes apartados:
.. completar la tabla asociando líneas de la Memoria Caché con direcciones de la MPrincipal (primera palabra de cada bloque)
+

[width="100%", cols="<4m,<4m,<4m,<4m,<4m"]
|================================
| Línea |Dirección MP |Dirección MP |Dirección MP |Dirección MP 
| 0 | 0x000000|0x010000 | |
| 1 | 0x000004| | |
|   |         | | | 
|2^14^ -1 |         | | | 
|================================
** la relación línea - dirección MP ( excepto tag) es una multiplicación x4, ya que cada línea y bloque tienen 4 bytes.
*** para última linea:  2^14^ -1 = 0x4000-1=0x3FFF -> 0x3FFF x 4 = 14 unos seguidos de 2_ceros(x4)= 0xFFFC
** Dirección 0x010000 -> los 4 digitos de menor peso (16bits), es decir, todos menos el tag, equivalen al bus de direcciones de la Caché, pero no direcciones de línea, sino de bytes, es decir la capacidad de la caché en bytes.
.. calcular la dirección lineal 0x0000B de la MP con dirección Bloque/Palabra se corresponde:
*** dirección bloque/palabra = dirección MP dividido por 4 -> el cociente es el número de bloque y el resto el número de palabra
*** 0x0000B  &#xF7; 4  -> 11=2x4+3  -> bloque Nº 2 y palabra Nº 3
.. calcular para la dirección MP 0x02ABCD con que dirección Bloque/Palabra se corresponde:
*** 0x02ABCD &#xF7; 4 -> muevo la coma dos posiciones a la izda (1/4=2^-2^) -> parte entera Bloque Nº 0xAAF3 y parte fracción Word Nº01
.. calcular al Bloque Nº 0xAAF3 qué línea le corresponde:
***  i= j mod m , donde m=2^14^ -> la línea es el resto de dividir 0xAAF3 entre 2^14^ -> parte fracción de mover la coma 14 posiciones a la izda -> 0x2AF3
.. calcular para la dirección MP 0x02ABCD con que byte y línea de la caché se corresponde:
*** byte -> 0xABCD 
*** línea -> 0xABCD &#xF7; 4  -> muevo la coma dos bits a la izda y tomo la parte entera -> 0x2AF3
.. calcular  para la dirección MP 0x02ABCD con que word de la caché se corresponde
*** word  -> 0xABCD en módulo 4 -> muevo la coma dos bits a la izda y tomo la parte fracción -> 01




. William Stalling Ejemplo 4.2a Pg125 : The system has a Cache memory of 64KB  and Main Memory of 16MB with a byte word size and four word block size. For a cache controller with direct mapping correspondence function search the main memory block addresses correspondences to cache memory *0x0CE7 number line* .


** Desarrollo:
*** Memoria principal: 16MB, 1word=1byte, 1block=4 byte. 
...
*** la dirección de 24 bits se descompone en 3 campos : etiqueta-línea-palabra
+
...
*** Cada tag agrupa 16K bloques -> (16K bloques/tag)x(4bytes/bloque)x(número de tags N)= CapacidadMemPrincipal=16MB -> 2^14^x2^2^xN=2^24^bytes -> N=2^8^ Tags -> Como cada agrupamiento Tag equivale a la capacidad de la Caché -> La MPrincipal equivale a 256 Cachés.

*** dirección línea 0CE7 :  dirección línea de 14 bits: 00-1100-1110-0111. Buscamos las direcciones de memoria asociadas a dicha línea.
**** Dirección MemPrincipal = Tag 0,   Linea 0CE7, Palabra 0 -> 0000-0000-00-1100-1110-0111-00 = 0000-0000-0011-0011-1001-1100 -> 00339C
**** Dirección MemPrincipal = Tag 1,   Línea 0CE7, Palabra 0 -> cambia el primer campo a 01 				       -> 01339C
**** Dirección MemPrincipal = Tag 255, Línea 0CE7, Palabra 0 -> cambia el primer campo a FF				       -> FF339C
**** Las direcciones de memoria de un bloque son la dirección de la primera palabra del bloque en la Memoria Principal.
...
** Sol:
*** Tag/Set/Word : 8/4/7
*** 0x7FAFA -> Tag 0xFF, Set 0x6, Word 0x7A. If Tag 0xFF is on any of the four lines of Set 0x6, then a "hit" has occurred.
...
. Consider a 32-bit microprocessor that has an on-chip 16-KByte four-way set-associative cache. Assume that the cache has a line size of four 32-bit words. 
...
. The Intel 80486 has an on-chip, unified cache. It contains 8 KBytes and has a four-way set-associative organization and a block length of four 32-bit words. The cache is organized into 128 sets. There is a single “line valid bit” and three bits, B0, B1, and B2
...
.  La arquitectura de un computador Intel tiene un bus del sistema con una frecuencia de reloj de 100MHz, el ancho del bus de datos son 64 bits y el ancho del bus de direcciones de la placa base es de 48 bits.
...
. On a typical Intel 8086-based system, connected via system bus to DRAM memory, for a read operation, RAS is activated by the trailing edge of the Address Enable signal (Figure 3.19). However, due to propagation and other delays, RAS does not go active until 50 ns after Address Enable returns to a low. Assume the latter occurs in the middle of the second half of state T1 (somewhat earlier than in Figure 3.19). Data are read by the processor at the end of T3. For timely presentation to the processor, however, data must be provided 60 ns earlier by memory. This interval accounts for propagation delays along the data paths (from memory to processor) and processor data hold time requirements. Assume a clocking rate of 10 MHz. 
